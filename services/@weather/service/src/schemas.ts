...
export const WeatherReading = Type.Object(
  {
    ...
    winddir_avg10m: Type.Integer({ description: "10-min avg wind direction (degrees)" }),
    ...
    baromrelin: Type.Number({ description: "Relative barometric pressure (inHg)" }),
    baromabsin: Type.Number({ description: "Absolute barometric pressure (inHg)" }),
    tempinf: Type.Number({ description: "Indoor temperature (Fahrenheit)" }),
    humidityin: Type.Integer({ description: "Indoor humidity (%)" }),
    feelsLike: Type.Number({ description: "Feels like temperature (Fahrenheit)" }),
...
export const HistoryQuery = Type.Object({
  start: Type.Optional(Type.String({ format: "date-time", description: "Start of range (ISO 8601)" })),
  end: Type.Optional(Type.String({ format: "date-time", description: "End of range (ISO 8601)" })),
  limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 10000, default: 288, description: "Max results (288 = 1 day at 5-min intervals)" })),
});
