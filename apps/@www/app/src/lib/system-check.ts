...
export async function performSystemCheck(): Promise<SystemCheckResult> {
  ...
  // Check Chrome AI API
  const languageModel = window.LanguageModel || (window as any).ai?.languageModel;
  const apiAvailable = !!languageModel;
  let aiStatus: 'unavailable' | 'downloading' | 'available' | 'unknown' = 'unknown';
  ...
  // Check Web Audio API
  const webAudioSupported = typeof AudioContext !== 'undefined' || typeof (window as any).webkitAudioContext !== 'undefined';
  ...
  const overallReady = isChrome && meetsMinimumVersion && webAudioSupported && geoSupported;
...
export function getSetupInstructions(check: SystemCheckResult): string[] {
  ...
  if (!check.browser.meetsMinimumVersion) {
    instructions.push(`⚠️  Chrome ${check.browser.version} detected. Chrome 127+ required.`);
  ...
  if (check.chromeAI.needsFlags) {
    ...
    instructions.push('');
    instructions.push('1. Visit: chrome://flags/#optimization-guide-on-device-model');
    ...
    instructions.push('   Click "Check for update" and wait for download (~1.7GB)');
    instructions.push('');
    instructions.push('5. Reload this page');
    instructions.push('');
    instructions.push('Note: Without AI, the app will use rule-based sound generation.');
    ...
    instructions.push('');
    instructions.push('   The app will work with rule-based generation until download completes.');
  ...
  if (!check.geolocation.supported) {
    ...
    instructions.push('   Enable in browser settings or use "debug" command for manual input.');
  }
